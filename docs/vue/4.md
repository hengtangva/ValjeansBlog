...
因此，想深入了解 vue-router ，我们先来了解一下 BOM 上的相关知识    
...
**当 url 的散列变化时，hashchange 事件就会被触发**    
...
所以，HTML5 新增了 window.history 对象，它的方法里实现了 改变 url 但不刷新页面的功能    
...
6. history.replace(obj,string,url)   将当前页面 url 在状态栈中，换成 replace 的 url    

**popstate 事件**    

每次，状态栈中的数据发生变化时，popstate 事件都会传递给 window 对象。       

如果，改变时是由 history.replace 或者 history.pushState 引起的，    
